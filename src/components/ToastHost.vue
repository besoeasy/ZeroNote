  ...
  <div class="fixed bottom-4 right-4 z-50 w-full max-w-sm px-4 sm:px-0">
    ...
    >
      <div
        v-for="t in toasts"
        :key="t.id"
        class="w-full rounded-2xl border shadow-lg backdrop-blur bg-white/90 overflow-hidden"
      ...
      >
        <div class="flex items-start gap-3 p-4">
          ...
          <div class="flex-1 min-w-0">
            ...
            <div class="mt-1 text-xs text-gray-600 leading-relaxed break-words">{{ t.message }}</div>
          </div>
        </div>
...
const toastBarClass = (type) => {
  if (type === "success") return "bg-emerald-500/70";
  if (type === "error") return "bg-red-500/70";
